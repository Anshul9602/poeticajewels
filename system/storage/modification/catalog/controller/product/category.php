	...
	public function index()
	{
		...
		if ($category_info) {
			...
			if ($category_info['image']) {
				$data['thumb'] = $this->model_tool_image->resize($category_info['image'], 1400, 450);
			} else {
				$data['thumb'] = '';
			}
			...
			foreach ($results as $result) {
				if ($result['image']) {
					if ($c1 == '87' || $c1 == '90') {
						$image = $this->model_tool_image->resize($result['image'], '500', '600');
					} else {
				...
				foreach ($results_img as $result_img) {
					if ($first) {
						if ($c1 == '87' || $c1 == '90') {
							$data['images'][$pid]['popup'] = $this->model_tool_image->resize($result_img['image'], '500', '600');
						} else {
...
if(file_exists('catalog/model/extension/fbcapidyad.php') && isset($this->request->get['path'])) { 
